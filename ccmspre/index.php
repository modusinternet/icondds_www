...
function CCMS_Set_LNG() {
	...
	if(PHP_VERSION_ID < 70300) {
		...
		setcookie("__Secure-ccms_lng", $CLEAN["ccms_lng"], time() + ($CFG["COOKIE_SESSION_EXPIRE"] * 60), "/;httponly=true;samesite=strict;secure=true", "", 0, 0);
	} else {
		setcookie("__Secure-ccms_lng", $CLEAN["ccms_lng"], [
		...
		if($row) {
			...
			if($a > $row["exp"]) {
				...
				if(PHP_VERSION_ID < 70300) {
					...
					setcookie("__Secure-ccms_session", $a, $c, "/;httponly=true;samesite=strict;secure=true", "", 0, 0);
				} else {
					setcookie("__Secure-ccms_session", $a, [
						'expires' => $c,
						'path' => "/",
						'domain' => $CFG["DOMAIN"],
						/*'samesite' => "strict",
